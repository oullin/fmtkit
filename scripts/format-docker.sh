...
ensure_formatter_image() {
	...
	local expected_version="go-fmt $VERSION"
	...
	if [[ -n "$build_reason" ]]; then
		...
		docker build --build-arg "VERSION=$VERSION" --label "local.go-fmt.formatter-fingerprint=$expected_fingerprint" -f "$dockerfile" -t "$image" "$REPO_ROOT"
	fi
}
...
run_formatter() {
	...
	docker run --rm \
		-v "$GO_FMT_PROJECT_DIR:/work" \
		-v "$GO_FMT_CACHE_VOLUME:/cache" \
		-w /work \
		-e "HOST_PROJECT_PATH=$GO_FMT_PROJECT_DIR" \
