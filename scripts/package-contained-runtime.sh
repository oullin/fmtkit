...
cp "$REPO_ROOT/cmd/fmt-ts" "$stage/support/cmd/fmt-ts"
cp "$REPO_ROOT/cmd/fmt-ts-files.sh" "$stage/support/cmd/fmt-ts-files.sh"
cp "$REPO_ROOT/cmd/fmt-lint" "$stage/support/cmd/fmt-lint"
cp "$REPO_ROOT/cmd/fmt-ts-lint.sh" "$stage/support/cmd/fmt-ts-lint.sh"
...
#!/usr/bin/env sh
root="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)"
export GO_FMT_SUPPORT_DIR="$root/support"
export GO_FMT_BLANK_LINES_SCRIPT="$root/support/scripts/blank-lines.ts"
export GO_FMT_FLUENT_CHAINS_SCRIPT="$root/support/scripts/fluent-chains.ts"
export GO_FMT_VALIDATE_SYNTAX_SCRIPT="$root/support/scripts/validate-syntax.ts"
export TSX_BIN="$root/bin/tsx"
export OXFMT_BIN="$root/bin/oxfmt"
exec "$root/support/cmd/fmt-ts" "$@"
SH
...
exec "$root/support/cmd/fmt-lint" "$@"
SH
