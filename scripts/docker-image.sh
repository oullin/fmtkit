...
case "$mode" in
	go)
		docker build --build-arg "VERSION=$VERSION" -f "$REPO_ROOT/docker/Dockerfile.go" -t "$GO_IMAGE" "$REPO_ROOT"
		;;
	node-ts)
		docker build --label "local.go-fmt.formatter-fingerprint=$FORMATTER_FINGERPRINT" -f "$REPO_ROOT/docker/Dockerfile.node-ts" -t "$NODE_TS_IMAGE" "$REPO_ROOT"
		;;
	full)
		docker build --build-arg "VERSION=$VERSION" --label "local.go-fmt.formatter-fingerprint=$FORMATTER_FINGERPRINT" -f "$REPO_ROOT/docker/Dockerfile.full" -t "$FULL_IMAGE" "$REPO_ROOT"
		;;
	clean)
		...
		docker volume rm "$GO_FMT_CACHE_VOLUME" 2>/dev/null || true
