...
write_executable "$bin_dir/fmtkit-sources" '#!/usr/bin/env bash
set -euo pipefail
printf "fmtkit-sources %s\n" "$*" >> "'"$log_file"'"
printf "fmtkit-sources-config %s %s %s\n" "${GIT_CONFIG_COUNT:-}" "${GIT_CONFIG_KEY_0:-}" "${GIT_CONFIG_VALUE_0:-}" >> "'"$log_file"'"
printf "sample.ts\0"'
...
(
	cd "$workdir"
	PATH="$bin_dir:$PATH" \
		FMTKIT_SUPPORT_DIR="$support_dir" \
		FMTKIT_SOURCES_BIN="$bin_dir/fmtkit-sources" \
		"$repo_root/infra/bin/fmtkit-lint" .
)

expected=$'fmtkit-sources .\nfmtkit-sources-config 1 safe.directory *\noxlint --config '"$support_dir"$'/.oxlintrc.json sample.ts'
actual="$(<"$log_file")"
